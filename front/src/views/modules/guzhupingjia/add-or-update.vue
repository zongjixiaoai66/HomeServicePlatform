    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="项目评价" prop="xiangmupingjia">
          <el-select :disabled="ro.xiangmupingjia" v-model="ruleForm.xiangmupingjia" placeholder="请选择项目评价">
            <el-option
      ...
      this.$http({
		...
		if(json.shouji!=''&&json.shouji){
          ...
          this.$message.error(data.msg);
            ...
            this.manyichengduOptions = "非常满意,满意,不满意".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `guzhupingjia/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `guzhupingjia/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.guzhupingjiaCrossAddOrUpdateFlag = false;
						 this.parent.search();
