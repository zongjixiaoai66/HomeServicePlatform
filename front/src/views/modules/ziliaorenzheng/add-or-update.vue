<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.shenfenzheng" label="身份证" prop="shenfenzheng">
          <file-upload
          tip="点击上传身份证"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.shenfenzheng?ruleForm.shenfenzheng:''"
          @change="shenfenzhengUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.shenfenzheng" label="身份证" prop="shenfenzheng">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.shenfenzheng.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.zigezhengshu" label="资格证书" prop="zigezhengshu">
          <file-upload
          tip="点击上传资格证书"
          action="file/upload"
          :limit="3"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.zigezhengshu" label="资格证书" prop="zigezhengshu">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.zigezhengshu.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
...
export default {
  ...
  methods: {
	...
	this.ruleForm.shenqingyuanyin = this.ruleForm.shenqingyuanyin.replace(reg,'../../../springbootc90g5/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.shenfenzheng!=null) {
		this.ruleForm.shenfenzheng = this.ruleForm.shenfenzheng.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `ziliaorenzheng/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.ziliaorenzhengCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    shenfenzhengUploadChange(fileUrls) {
	this.ruleForm.shenfenzheng = fileUrls;
	this.addEditUploadStyleChange()
    },
